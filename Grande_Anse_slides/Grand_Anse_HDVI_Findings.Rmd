...
ie <- function(cutoff, var) {
  ...
  nrow(tot_in_nv) / nrow(tot_in)
}

ee <- function(cutoff, var) {
  tot_v <- Gran_Anse_1 %>% filter(.data[[var]] == "Never")
  tot_v_ex <- tot_v %>% filter(HDVI < cutoff)
  nrow(tot_v_ex) / nrow(tot_v)
}

in_ex1 <- data.frame(cutoff = seq(0.1, 0.9, 0.1)) %>%
  mutate(label = paste("HDVI:", cutoff))
in_ex1$in_err <- sapply(in_ex$cutoff, ie, var = "Absence_Food")
in_ex1$ex_err <- sapply(in_ex$cutoff, ee, var = "Absence_Food")

ggplot(in_ex1, aes(in_err, ex_err)) +
  geom_point(color = "cornflowerblue", size = 2) +
  geom_text(aes(label = cutoff), color = "cornflowerblue", nudge_x = 0.02, nudge_y = 0, check_overlap = T) +
  geom_text(aes(x = 0.4, y = 0.35, label = "label = HDVI cutoff"), color = "grey") +
  theme_minimal() +
  theme(panel.grid.minor = element_blank()) +
  labs(subtitle = "* vulnerable households defined as those who often experience absence of food",
       x = "inclusion error", y = "exclusion error")
...
in_ex2 <- data.frame(cutoff = seq(0.1, 0.9, 0.1)) %>%
  mutate(label = paste("HDVI:", cutoff))
in_ex2$in_err <- sapply(in_ex$cutoff, ie, var = "Restricted_Food")
in_ex2$ex_err <- sapply(in_ex$cutoff, ee, var = "Restricted_Food")
...
in_ex3 <- data.frame(cutoff = seq(0.1, 0.9, 0.1)) %>%
  mutate(label = paste("HDVI:", cutoff))
in_ex3$in_err <- sapply(in_ex$cutoff, ie, var = "Bed_Hungery")
in_ex3$ex_err <- sapply(in_ex$cutoff, ee, var = "Bed_Hungery")
...
ggplot(in_ex3, aes(in_err, ex_err)) +
  ...
  theme_minimal() +
  theme(panel.grid.minor = element_blank()) +
  labs(subtitle = "* vulnerable households defined as those who often go to bed hungry",
       x = "inclusion error", y = "exclusion error")
